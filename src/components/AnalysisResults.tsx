...
export const AnalysisResults: React.FC<AnalysisResultsProps> = ({
  ...
  const generateHumanReadableReport = () => {
    ...
    const simpleVerdict = isReal ? 'REAL / AUTHENTIC PHOTOGRAPH' : 'AI-GENERATED / SYNTHETIC MEDIA';

    const simpleSummary = isReal
      ? 'This image is a real photograph taken with a physical camera or smartphone. It is NOT generated by AI.'
      : 'This image was created or heavily altered using Artificial Intelligence (AI). It is NOT an authentic camera photograph.';

    const deviceNote = fileName.toLowerCase().includes('whatsapp') || forensics.metadata_status?.toLowerCase().includes('whatsapp')
      ? 'Camera EXIF details were removed by WhatsApp when sharing (standard privacy feature). This is completely normal for messaging apps and does NOT mean the image is fake.'
      : (forensics.camera_details || (isReal ? 'Physical smartphone / camera sensor' : 'AI Image Generator'));

    return `================================================================================
                    AI TRUTHLENS FORENSIC AUTHENTICITY REPORT
================================================================================

Scan Date & Time : ${scanDate}
File Name        : ${fileName}
Media Type       : ${result.media_type ? result.media_type.toUpperCase() : 'IMAGE'}

--------------------------------------------------------------------------------
1. VERDICT SUMMARY (IN PLAIN LANGUAGE)
--------------------------------------------------------------------------------
Verdict          : ${simpleVerdict}
Confidence       : ${confidence.toFixed(1)}% Sure
...
--------------------------------------------------------------------------------
3. WHAT WAS CHECKED & WHAT WE FOUND
--------------------------------------------------------------------------------
• Real Camera Probability : ${forensics.authenticity_score?.toFixed(1) || (isReal ? '94.0' : '5.0')}%
  -> Higher means strong physical camera sensor features.

• AI Generation Likelihood : ${forensics.calibrated_neural_score?.toFixed(1) || (isReal ? '6.0' : '95.0')}%
  -> Higher means patterns typical of generative AI models (like Midjourney, DALL-E, or Stable Diffusion).

• Camera Hardware & Source :
  -> ${deviceNote}

• Texture & Sensor Grain (Noise) :
  -> Status: ${forensics.noise_status || (isReal ? 'Natural Photon Grain' : 'Synthetic Smoothing')}
  -> Explanation: Real cameras leave natural microscopic grain when capturing light. AI software usually leaves an unnaturally smooth, plastic, or airbrushed surface.

• Compression & Image Encoding :
  -> Status: ${forensics.compression_status || 'Standard JPEG Profile'}

--------------------------------------------------------------------------------
4. FINAL CONCLUSION
--------------------------------------------------------------------------------
${isReal
  ? 'VERIFIED AS AUTHENTIC: All physical visual indicators (lighting, natural skin textures, reflections, and environmental geometry) match a real photo taken in the real world.'
  : 'VERIFIED AS AI: The visual features and digital patterns match artificial intelligence generation rather than an authentic camera capture.'}

================================================================================
Report generated by AI TruthLens Forensic Suite
...
`;
  ...
  const copySummary = () => {
    const isReal = result.result === 'REAL';
    const text = `AI TruthLens Authenticity Report
File: ${fileName}
Result: ${isReal ? 'REAL (Authentic Photograph)' : 'AI-GENERATED (Synthetic Media)'}
...
Details: ${result.explanation}`;
  ...
  const downloadReport = () => {
    ...
    a.download = `truthlens_report_${safeName}.txt`;
    a.click();
  ...
  const downloadJson = () => {
    ...
    a.href = url;
    const safeName = fileName.replace(/[^a-zA-Z0-9]/g, '_');
    a.download = `truthlens_data_${safeName}.json`;
    a.click();
    URL.revokeObjectURL(url);
  };
    ...
    <div className="rounded-3xl bg-slate-900/60 border border-slate-800 p-6 sm:p-8 space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-slate-800">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            id="download-human-report-btn"
            onClick={downloadReport}
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/30 text-blue-300 hover:text-white text-xs font-semibold transition"
            title="Download plain-English human readable report (.txt)"
          >
          ...
          <button
            id="download-json-data-btn"
            onClick={downloadJson}
            className="flex items-center gap-1.5 px-2.5 py-2 rounded-xl bg-slate-800/70 hover:bg-slate-700/80 text-slate-400 hover:text-slate-200 text-xs font-medium transition"
            title="Download raw technical data (.json)"
          >
